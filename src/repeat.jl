...
function Base.repeat(x::AbstractArray, (left, right)::Pattern; context...)
    ...
    repeat_dim_names = @ignore_derivatives setdiff(right_names, left_names)
    context_repeat = NamedTuple(d => context[d] for d in repeat_dim_names)
    info_dim_names = @ignore_derivatives setdiff(keys(context), repeat_dim_names)
    context_info = NamedTuple(d => context[d] for d in info_dim_names)
    @ignore_derivatives isempty(setdiff(right_names, left_names, keys(context))) || throw(ArgumentError("Unknown dimension sizes: $(setdiff(right_names, left_names))"))
    expanded = reshape_in(x, left; context_info...)
    right_names_no_repeat = @ignore_derivatives setdiff(right_names, repeat_dim_names)
