...
function Base.reduce(f::Function, x::AbstractArray, (left, right)::ArrowPattern; context...)
    ...
    reduced_dims::NTuple{length(left_names)-length(right_names),Int}, reduced_left_names = @ignore_derivatives begin
        ...
        reduced_dims, ntuple(i -> reduced_left_names[i], length(right_names))
    end
    reduced = f(expanded, dims=reduced_dims)
    dropped = dropdims(reduced, dims=reduced_dims)
    permuted = permute(dropped, reduced_left_names, right_names)
